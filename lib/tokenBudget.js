/**
 * Token Budget System for ihavefriend.com
 * 
...
const BUDGET_CONFIG = {
  totalTokens: 800,
  phases: {
    alwaysOn:       { ratio: 0.31, maxMemories: 8  },
    contextual:     { ratio: 0.44, maxMemories: 5  },
    proactive:      { ratio: 0.15, maxMemories: 2  },
    background:     { ratio: 0.10, maxMemories: 3  },
  },
...
async function getAlwaysOnMemories(db, userId, budget) {
  ...
  return selectWithinBudget(rows, budget);
}
...
async function vectorSimilaritySearch(db, userId, message, limit) {
  ...
  const rows = await db.query(`
    SELECT 
      ...
      1 - (me.embedding <=> $2) AS similarity
...
async function getProactiveMemories(db, userId, budget) {
  ...
  const scored = [...upcoming, ...routine].map(node => {
    let urgency = 1;
...
async function getBackgroundMemories(db, userId, budget) {
  ...
  const rows = await db.query(`
    ...
    LIMIT $2
  `, [userId, BUDGET_CONFIG.phases.background.maxMemories]);
 ...
 * (Placeholder — actual implementation uses Gemini text-embedding-004)
 */
