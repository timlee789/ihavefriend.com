...
async function decideNextAction(input, apiKey) {
  ...
  const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        temperature: 0.3,
        // Tim verification round 2: 1500 still truncated #2 and #3
        //   mid-rationale ("...로 깊" / "...alternative_action: \""
        //   tails). responseSchema forces all 6 keys to be present,
        //   which raises minimum output length, and KO rationale
        //   tokens are heavy (≈ 1.5–2 tokens/char). 2500 leaves
        //   comfortable headroom; the prompt now caps rationale at
        //   50 chars so this remains a safety margin, not a target.
