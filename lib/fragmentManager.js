...
async function getFragments(db, userId, options = {}) {
  ...
  let query = `
    SELECT
      ...
      -- Beta Step 2b (2026-05-21) — Tim 의 카드 배지 버그 수정.
      --   이전: 카드가 f.book_id (자서전 user_books 연결) 만 봐서, collection
      --   (이야기책 챕터) 에 넣은 fragment 는 배지가 안 떴음.
      --   해결: 소속 collections 를 json_agg 로 동봉 → 카드가 챕터 이름 표시.
      COALESCE((
        SELECT json_agg(
                 json_build_object('id', col.id, 'name', col.name)
                 ORDER BY col.display_order, col.created_at
               )
          FROM collection_fragments cf
          JOIN user_collections col ON col.id = cf.collection_id
