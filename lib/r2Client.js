...
/**
 * 🔥 2026-05-06 R0 — Photobook v3 R0 (original-photo storage).
 *
 * Builds the R2 key for the high-resolution original of a photobook page
 * photo. Same prefix as makePhotoKey() so both objects sit in the same
 * `photos/u_X/p_Y/` folder in R2 console — easy to spot the pair.
 *
 * Difference: the filename is `photo_orig_{ts}.{ext}` so the two objects
 * don't collide when a user replaces a photo (the timestamp also keeps
 * each upload distinct).
 *
 * @param {number|string} userId - User.id
 * @param {string} pageId - PhotobookPage.id (UUID)
 * @param {string} ext - File extension without dot (jpg in R0; client
 *   converts HEIC → JPEG via canvas before upload).
 */
export function makeOriginalPhotoKey(userId, pageId, ext = 'jpg') {
  return `photos/u_${userId}/p_${pageId}/photo_orig_${Date.now()}.${ext}`;
}
