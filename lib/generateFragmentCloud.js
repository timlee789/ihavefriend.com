...
async function generateFragmentCloud({
  ...
  // 🔥 Task 80 — when chat/end has applied a Whisper-recovery
  //   override, all of the user side arrives as ONE long message
  //   while Emma's questions still appear interleaved with placeholder
  //   roles in the conversationText. The model can confuse this for
  //   a one-shot statement and ignore the assistant's questions. We
  //   detect this shape (single user msg, ≥1000 chars) and prepend a
  //   short hint so the model treats the user message as the answer
  //   source for Emma's questions.
  const whisperAggregate =
    userMessages.length === 1 && userMessages[0].length >= 1000;
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            // 🔥 Task 79 — bumped from 16384 to 24576. Tim's longest
            //   STORY transcripts (Korean ~10000+ chars across multiple
            //   turns) were hitting MAX_TOKENS partway through fragment
            //   reassembly, surfacing as a truncated body in /my-stories.
            //   Per-call cost difference is negligible vs. the UX cost
            //   of a chopped story.
...
function buildFragmentPrompt({ elements, conversationText, userMessagesText, lang, conversationMode = 'auto', topicAnchor = null, isContinuation = false, parentTitle = null, whisperAggregate = false }) {
  const el = elements;
...
- Tags: only include items actually in user's messages. Empty arrays if nothing fits.
  ...
  // 🔥 Task 80 — when chat/end has merged the user side into a single
  //   long aggregate (Whisper recovery transcript), tell the model
  //   explicitly. Otherwise it can read the conversation as a flat
  //   monologue and lose Emma's questions entirely, or worse, treat
  //   her questions as the user speaking.
  const whisperAggregateBlock = whisperAggregate ? `════════════════════════════════════════════════
🎤 USER SIDE IS A SINGLE WHISPER-RECOVERED AGGREGATE
════════════════════════════════════════════════
The user-side text below is ONE long message that aggregates
everything the user said across the whole session (recorded by
the user's microphone and transcribed by Whisper). Emma's questions
appear in the conversation context to show WHEN she asked things.
Treat the user message as the answer source for Emma's questions
and weave the answers into a coherent first-person account.
...
${whisperAggregateBlock}${interviewHeader}${topicAnchorBlock}${isContinuation ? `════════════════════════════════════════════════
