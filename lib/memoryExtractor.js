...
- DO extract: names, relationships, health issues, job/projects, hobbies, goals, upcoming events
...
async function saveMemories(db, userId, memories, geminiApiKey) {
  ...
  for (const memory of memories) {
    ...
    if (existing.rows.length > 0) {
      ...
      // Track discussion_depth and times_discussed if columns exist (best-effort)
      try {
      ...
      // Try insert with discussion tracking columns; fall back without them
      let inserted;
      try {
