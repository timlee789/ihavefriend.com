...
async function analyzeAnswer(input, apiKey) {
  ...
  const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        temperature: 0.2,
        // 🔥 Task 88 follow-up: 600 was truncating mid-JSON whenever
        //   mentioned_details + ungrounded_topics + answer_summary
        //   filled out in Korean (1 KO char ≈ 1.5–2 tokens). 1500
        //   leaves comfortable headroom for the richest scenarios
        //   without inviting runaway output (the prompt caps summary
        //   at 30 chars and depth=3 answers max ~5 dims).
