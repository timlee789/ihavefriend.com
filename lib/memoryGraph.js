 ...
 *  - confidence reads/writes go through enumMappers
 *  - PostgreSQL CASE requires explicit ::"Confidence" cast for enum values
 *  - SELECT results are converted back to snake_case for biz-logic compatibility
 */
...
async function applyMemoryDecay(db, userId) {
  ...
  // v2: Enum values are SCREAMING_CASE; parameterized + explicit cast required.
  const result = await db.query(`
    UPDATE memory_nodes SET
      confidence = CASE
        WHEN last_mentioned < NOW() - INTERVAL '90 days' THEN $2::"Confidence"
        WHEN last_mentioned < NOW() - INTERVAL '30 days' AND confidence = $3::"Confidence" THEN $4::"Confidence"
        ELSE confidence
      END
    WHERE user_id = $1 AND is_active = true
  ...
  `, [
    ...
    confidenceToDb('high'),    // 'HIGH'
    confidenceToDb('medium'),  // 'MEDIUM'
  ]);

  // Convert confidence back to snake_case for downstream biz-logic compatibility
...
async function getFullGraph(db, userId) {
  ...
  return {
    nodes: nodes.rows.map(n => ({
      ...n,
      confidence: confidenceFromDb(n.confidence),
    })),
    edges: edges.rows,
    stats: {
